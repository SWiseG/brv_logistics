    ...
    displayResults: function(results, container) {
        if (!results || results.length === 0) {
            ...
            const html = results.map(product => `
                <a href="/produtos/${product.slug}/" class="dropdown-item d-flex align-items-center">
                    <img src="${product.image}" alt="${product.name}" class="mr-3" style="width: 40px; height: 40px; object-fit: cover;">
                    <div>
                        <div class="font-weight-bold">${product.name}</div>
                        <small class="text-muted">${utils.formatPrice(product.price)}</small>
                    </div>
                </a>
            `).join('');
...
const ProductFilters = {
    init: function() {
        ...
        filterInputs.forEach(input => {
            input.addEventListener('change', utils.debounce(this.applyFilters.bind(this), 500));
        });
    ...
    loadFilteredProducts: function(params) {
        ...
        const hideLoading = utils.showLoading(productsContainer);

        fetch(`/produtos/filter/?${params.toString()}`)
        ...
        .catch(error => {
            ...
            utils.notify('Erro ao carregar produtos', 'error');
        })
        .finally(() => {
            hideLoading();
        });
    ...
    initPriceRange: function() {
        ...
        const priceRangeDisplay = document.querySelector('#priceRangeDisplay');

        if (!minPriceInput || !maxPriceInput) return;

        function updateDisplay() {
            const min = utils.formatPrice(minPriceInput.value || 0);
            const max = utils.formatPrice(maxPriceInput.value || 9999);
